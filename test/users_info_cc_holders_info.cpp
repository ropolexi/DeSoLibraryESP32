...
void setup()
{
  ...
  getExchangeRates();
  getProfile(username);
  Serial.println("****TOP CC holders Information***");
  deso.eraseUsers();
  for (int i = 0; i < 10; i++)
  {
    if (strlen(profile1.TopHodlersUserNames[i]) > 0)
    {
      if(strcmp(profile1.TopHodlersUserNames[i],username)==0){
      continue;
      }
      
      deso.addUser(profile1.TopHodlersUserNames[i]);
  ...
  for(int i=0;i<deso.users.size();i++){
    Serial.print(i + 1);
    Serial.print(": ");
    Serial.print(deso.users[i].username);
    getProfile(deso.users[i].username);
  }
  Serial.println("*******");
}
void loop()
